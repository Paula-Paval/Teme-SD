...
class AuthService: IAuthService {
    ...
    fun encodePassword(password: String, encodingId:String):MyCustomResponse<Any>{
        try{
// returneaza encoder-ul de parole conform algoritmuluispecificat
            val passwordEncoder =
...
//id invalid al encoder-ului specificat
    ...
    override fun matchPassword(matchModel: MatchPasswordModel):MyCustomResponse<Any> {
        return try{
// adaugam encodingId-ul in parola hash-uita pentruafidelegat un PasswordEncoder corespunzator
            val currentHashPasswordWithAlg =
                "{${matchModel.encodingId}}${matchModel.hashPassword}"
            MyCustomResponse(successfulOperation = true,
                code=200,
