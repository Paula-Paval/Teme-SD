...
class AgendaController {
    ...
    fun createPerson( @RequestParam("id") id:String, @RequestParam("lastName") lastName: String, @RequestParam("fistName") fistName: String, @RequestParam("telephoneNumber") telephoneNumber:String):ResponseEntity<Person> {
        var person=Person(id.toInt(), lastName, fistName, telephoneNumber)
        _agendaService.createPerson(person)
        return ResponseEntity(person, HttpStatus.CREATED)
    }
    @RequestMapping(value = ["/person"], method =[RequestMethod.GET])
    fun getPerson(@RequestParam("id") id:String): ResponseEntity<Person?>{
        val person: Person? = _agendaService.getPerson(id.toInt())
    ...
    fun patchPerson(@PathVariable id: Int, @RequestBody patchOperations: JsonPatch): ResponseEntity<Unit> {
        _agendaService.getPerson(id)?.let {
            // aplicam operatiile de Patch peste obiectul gasitval objectMapper = ObjectMapper()
    ...
    fun search(@RequestParam(required = false, name = "lastName",defaultValue = "") lastName: String,@RequestParam(required = false, name = "firstName",defaultValue = "") firstName: String,
